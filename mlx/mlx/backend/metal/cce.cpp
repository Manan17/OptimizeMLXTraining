...
void CCELoss::eval_gpu(
  ...
  if (use_chunked_forward) {
    ...
    int num_chunks = (V + CHUNK_V - 1) / CHUNK_V;
    int max_chunk_v = std::min(CHUNK_V, V);
    ...
    for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
      int v_start = chunk_idx * CHUNK_V;
      int v_end = std::min(v_start + CHUNK_V, V);
      int chunk_v = v_end - v_start;

      // Create view of weight_chunk
      array weight_chunk({chunk_v, H}, w.dtype(), nullptr, {});
      int64_t w_offset = static_cast<int64_t>(v_start) * H;
      weight_chunk.copy_shared_buffer(
          w, {static_cast<int64_t>(H), 1}, w.flags(), static_cast<size_t>(chunk_v * H), w_offset);

      // Create view of logits_chunk for actual chunk size
      // NATIVE BF16: logits_view uses same dtype as logits_chunk (BF16 or FP32)
      array logits_view({N, chunk_v}, logits_dtype, nullptr, {});
      logits_view.copy_shared_buffer(
          logits_chunk, {static_cast<int64_t>(chunk_v), 1}, logits_chunk.flags(),
          static_cast<size_t>(N * chunk_v), 0);
      ...
      compute_encoder.set_bytes(chunk_v, 6);
  ...
  if (use_mma) {
    ...
    if (use_saved_lse) {
      ...
      for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
        ...
        array logits_view({N, chunk_v}, float32, nullptr, {});
        logits_view.copy_shared_buffer(
            lse_logits_chunk, {static_cast<int64_t>(chunk_v), 1}, lse_logits_chunk.flags(),
            static_cast<size_t>(N * chunk_v), 0);
    ...
    for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
      ...
      array d_logits_view({N, chunk_v}, float32, nullptr, {});
      ...
      array grad_weight_chunk({chunk_v, H}, float32, nullptr, {});
      ...
      grad_weight_chunk.copy_shared_buffer(
          ...
          static_cast<size_t>(chunk_v * H), gw_offset);
